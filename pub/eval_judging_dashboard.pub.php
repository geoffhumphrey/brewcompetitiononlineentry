...
if (TESTING) {
	...
	elseif ($_SESSION['prefsStyleSet'] == "BJCP2026") $db_conn->where ("brewStyleVersion = ? OR brewStyleVersion = ? OR brewStyleVersion = ?", array("BJCP2026","BJCP2025","BJCP2021"));
...
if ((!empty($assigned_score)) && (count($assigned_score) > 1)) {
	...
	if (count(array_unique($assigned_score)) === 1) $notes .= "<div class=\"text-success mb-1\">".$evaluation_info_026."<br>".$label_assigned_score.": ".$assigned_score[0]."</div>";
	
	else {
		$notes .= "<div class=\"text-danger mb-1\"><strong>".$evaluation_info_017."<br>";
	...
	if ($judging_open) { 
		...
		if ($disable_add_edit) {
			...
			if (TESTING) {
				...
				$actions .= "<a onclick=\"localStorage.clear();\" class=\"btn btn-block btn-sm btn-secondary\" href=\"".$add_link_full."\">Classic</a>";
				if ($row_style['brewStyleType'] <= 3) $actions .= "<a onclick=\"localStorage.clear();\" class=\"btn btn-block btn-sm btn-secondary\" href=\"".$add_link_structured."\">Structured</a>";
				if (($row_style['brewStyleType'] == 1) || ($row_style['brewStyleType'] == 2)) {
					if ($row_style['brewStyleType'] == 1) $actions .= "<a onclick=\"localStorage.clear();\" class=\"btn btn-block btn-sm btn-secondary\" href=\"".$add_link_checklist."\">Checklist</a>";
					if ($row_style['brewStyleType'] == 2) $actions .= "<a onclick=\"localStorage.clear();\" class=\"btn btn-block btn-sm btn-secondary\" href=\"".$add_link_nw_cider."\">NW Cider</a>";
				}
				...
				$actions .= "<div class=\"d-grid mb-2\">";
				$actions .= "<a onclick=\"localStorage.clear();\" class=\"btn btn-sm btn-primary\" href=\"".$add_link."\">".$label_add."</a>";
				$actions .= "</div>";
			}
