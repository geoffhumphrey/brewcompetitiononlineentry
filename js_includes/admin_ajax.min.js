...
function save_column(ajax_url, column, action, id, rid1, rid2, rid3, rid4, element_id, output = "value") {
    ...
    $("#" + element_id + "-" + column + "-status").show();
    $("#" + element_id + "-" + column + "-status-msg").show();
    let url = ajax_url + "save.ajax.php?action=" + action + "&go=" + column + "&id=" + id;
    if (rid1 != "default") {
        url += "&rid1=" + rid1
    }
    if (rid2 != "default") {
...
function select_place(ajax_url, column, action, id, rid1, rid2, rid3, rid4, element_id) {
    if (($('select option[value="' + $('#' + element_id).val() + '"]:selected').length > 1) && $('select option[value="' + $('#' + element_id).val() + '"]:selected') !== "") {
        ...
        save_column(ajax_url, column, action, id, rid1, rid2, rid3, rid4, element_id)
    }
    if ($('#' + element_id).val() === "") {
        save_column(ajax_url, column, action, id, rid1, rid2, rid3, rid4, element_id)
    }
...
function select_place_multi(ajax_url, column, action, id, rid1, rid2, rid3, rid4, element_id) {
...
function regenerate_judging_numbers(ajax_url, action, source, element_id) {
    $("#" + element_id + "-status").removeClass();
    ...
    $("#" + element_id + "-status").show();
    $("#" + element_id + "-status-msg").show();
    var url = ajax_url + "regenerate.ajax.php?action=" + action;
    jQuery.ajax({
        url: url,
        success: function (data) {
            var jsonData = JSON.parse(data);
...
function purge_data(ajax_url, action, go, source, element_id) {
    $("#" + element_id + "-status").removeClass();
    $("#" + element_id + "-status").attr("class", "fa fa-lg fa-cog fa-spin text-muted");
    $("#" + element_id + "-status-msg").html("Processing...");
    $("#" + element_id + "-status-msg").attr("class", "text-muted small");
    ...
    let url = ajax_url + "purge.ajax.php?action=" + action + "&go=" + go;
    let data_const = "";
        ...
        success: function (data) {
            ...
            if (jsonData.status === "0") {
                ...
                if ((scored_places_discrepency) && (scored_places_discrepency_count > 0)) {
                    ...
                    if ((section == "evaluation") || (go == "judging_scores")) {
                        import_status_discrepency += "</p><br><br>"
                    }
                    import_status_discrepency += "<div id=\"discrepency-table\" class=\"row\">";
                    import_status_discrepency += "<div class=\"col col-xs-12 col-sm-12 col-md-12 col-lg-8\">";
                    import_status_discrepency += "<table style=\"background-color: #ededed; margin-top: 15px;\" class=\"table table-bordered table-s" +
                            "triped table-hover\">";
