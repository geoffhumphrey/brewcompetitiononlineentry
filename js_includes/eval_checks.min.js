function checkConsensus(data) {
    ...
    data.forEach(function (item, index) {
        if ((entered_consensus_score > 0) && (entered_consensus_score !== item)) {
            disparity += 1
        }
    });
    if (disparity > 0) {
...
function displayCalc(score) {
    ...
    if ((score > 0) && (score <= 13)) {
        $("#scoring-guide").html(score_problematic);
        $("#scoring-guide-bottom-prob").attr("class", "row scoring-guide-bottom-text bg-danger text-white")
    } else if ((score > 13) && (score <= 20)) {
        $("#scoring-guide").html(score_fair);
        $("#scoring-guide-bottom-fair").attr("class", "row scoring-guide-bottom-text bg-warning text-dark")
    } else if ((score > 20) && (score <= 29)) {
        $("#scoring-guide").html(score_good);
        $("#scoring-guide-bottom-good").attr("class", "row scoring-guide-bottom-text bg-info text-dark")
    } else if ((score > 29) && (score <= 37)) {
        $("#scoring-guide").html(score_very_good);
        $("#scoring-guide-bottom-v-good").attr("class", "row scoring-guide-bottom-text bg-primary text-white")
    } else if ((score > 37) && (score <= 44)) {
        $("#scoring-guide").html(score_excellent);
        $("#scoring-guide-bottom-excellent").attr("class", "row scoring-guide-bottom-text bg-success-subtle text-success-emphasis")
    } else if ((score > 44) && (score <= 50)) {
        $("#scoring-guide").html(score_outstanding);
        $("#scoring-guide-bottom-outstanding").attr("class", "row scoring-guide-bottom-text bg-success text-white")
    }
};
...
function checkScoreRange(score, data, score_range, method) {
    var disparity = 0;
    data.forEach(function (item, index) {
        if (Math.abs(item - score) > score_range) {
            disparity += 1
        }
    });
    if (disparity > 0) {
