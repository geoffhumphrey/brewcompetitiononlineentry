...
// This function is duplicated in the app.pub.js file
// Move to a shared file
function fetchRecordCount(ajax_url,element,is_class,db_table,p1,c1,p2,c2,p3,c3) {
    ...
    // Build URL
    count_url = ajax_url + "count_records.ajax.php?section=" + db_table;
    if ((typeof p1 !== 'undefined') && (typeof c1 !== 'undefined')) count_url = count_url + "&p1=" + p1 + "&c1=" + c1;
    if ((typeof p2 !== 'undefined') && (typeof c2 !== 'undefined')) count_url = count_url + "&p2=" + p2 + "&c2=" + c2;
    if ((typeof p3 !== 'undefined') && (typeof c3 !== 'undefined')) count_url = count_url + "&p3=" + p3 + "&c3=" + c3;
...
function check_user_name_avail(ajax_url,element_id,status_id,type) {
    ...
    jQuery.ajax({
        url: ajax_url+"account_checks.ajax.php?action=username"+append,
...
function check_security_question_answer(ajax_url,element_id,status_id) {
   
    var element_id = "#"+element_id;
    var status_id = "#"+status_id;

    jQuery.ajax({
        url: ajax_url+"account_checks.ajax.php?action=check_answer",
...
function check_valid_email(ajax_url,element_id,status_id) {
    
    var element_id = "#"+element_id;
    var status_id = "#"+status_id;

    jQuery.ajax({
        url: ajax_url+"account_checks.ajax.php?action=email",
...
/**
 * ------------------------------
 * V3: BS Collapse State Manager
 * ------------------------------
 * 
 * Initialize the collapse state manager for a specific element
 * @param {Object} config - Configuration object containing element IDs and classes
 * @param {string} config.collapseElementId - ID of the collapsible element
 * @param {string} config.toggleButtonId - ID of the toggle button
 * @param {string} config.toggleIconId - ID of the icon element that changes when collapsed/expanded
 * @param {string} config.iconClassExpanded - Class to apply to the icon when expanded
 * @param {string} config.iconClassCollapsed - Class to apply to the icon when collapsed
 */

function initCollapseStateManager(config) {
  const collapseElement = document.getElementById(config.collapseElementId);
  const toggleBtn = document.getElementById(config.toggleButtonId);
  const toggleIcon = document.getElementById(config.toggleIconId);
  
  // Create a unique cookie name based on the element ID
  const cookieName = `collapse_state_${config.collapseElementId}`;
  
  // Check if we have a saved state on page load
  const savedState = getSessionCookie(cookieName);
  
  if (savedState === 'collapsed') {
    // Collapse the element based on cookie
    $(collapseElement).collapse('hide');
    // Update the icon
    toggleIcon.classList.remove(config.iconClassExpanded);
    toggleIcon.classList.add(config.iconClassCollapsed);
  } else if (savedState === 'expanded') {
    // Expand the element based on cookie
    $(collapseElement).collapse('show');
    // Update the icon
    toggleIcon.classList.remove(config.iconClassCollapsed);
    toggleIcon.classList.add(config.iconClassExpanded);
  }
  
  // Listen for Bootstrap's collapse events to update the cookie
  $(collapseElement).on('hidden.bs.collapse', function () {
    setSessionCookie(cookieName, 'collapsed');
    toggleIcon.classList.remove(config.iconClassExpanded);
    toggleIcon.classList.add(config.iconClassCollapsed);
  });
  
  $(collapseElement).on('shown.bs.collapse', function () {
    setSessionCookie(cookieName, 'expanded');
    toggleIcon.classList.remove(config.iconClassCollapsed);
    toggleIcon.classList.add(config.iconClassExpanded);
  });
}
...
$(document).ready(function () {
    ...
    $("#forgot-user-name").keyup(function(e) {
        if ($("#forgot-user-name").val() != "") {
            ...
            if (e.which == 13) {
                check_user_name_avail(ajax_url,'forgot-user-name','forgot-user-name-status','forgot_password');
            }
        }
        if ($("#forgot-user-name").val() == "") {
            forgot_password_hide_elements();
    ...
    $("#forgot-user-name-status").keyup(function(e) {
        if ($("#security-question-answer").val() != "") {
            ...
            //
            if (e.which == 13) {
                check_security_question_answer(ajax_url,'security-question-answer','security-question-response-status')
            }
        }
        if ($("#security-question-answer").val() == "") {
            $("#forgot-user-name-check-button").prop("disabled",false);
