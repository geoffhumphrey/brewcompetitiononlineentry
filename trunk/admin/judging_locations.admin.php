...
<tbody>
 ...
 <tr>
  ...
  <td width="15%" class="dataList"><?php echo getTimeZoneDateTime($_SESSION['prefsTimeZone'], $row_judging_locs['judgingDate'], $_SESSION['prefsDateFormat'],  $_SESSION['prefsTimeFormat'], "long", "date"); ?></td>
  <td width="15%" class="dataList"><?php echo getTimeZoneDateTime($_SESSION['prefsTimeZone'], $row_judging_locs['judgingDate'], $_SESSION['prefsDateFormat'],  $_SESSION['prefsTimeFormat'], "long", "time-gmt"); ?></td>
...
<table>
  <tr>
    ...
    <td class="data"><input id="judgingDate" name="judgingDate" type="text" size="20" value="<?php if ($action == "edit") echo getTimeZoneDateTime($_SESSION['prefsTimeZone'], $row_judging['judgingDate'], $_SESSION['prefsDateFormat'],  $_SESSION['prefsTimeFormat'], "system", "date"); ?>"></td>
    <td class="data"><span class="required">Required</span></td>
  </tr>
  <tr>
    <td class="dataLabel">Start Time:</td>
    <td class="data"><input id="judgingTime" name="judgingTime" size="10" value="<?php if ($action == "edit") echo getTimeZoneDateTime($_SESSION['prefsTimeZone'], $row_judging['judgingDate'], $_SESSION['prefsDateFormat'],  $_SESSION['prefsTimeFormat'], "system", "time"); ?>"></td>
    <td class="data"><span class="required">Required</span></td>
  </tr>
  <tr>
	 ...
	 $(document).ready(function() {
		$('#sortable').dataTable( {
			...
			"aoColumns": [
				...
				<?php if (($filter == "stewards") || ($filter == "bos")) { ?>
				{ "asSorting": [  ] },
				null,
				null,
				null,
...
<form name="form1" method="post" action="<?php echo $base_url; ?>includes/process.inc.php?action=update&amp;dbTable=<?php echo $brewer_db_table; ?>&amp;filter=<?php echo $filter; ?>">
<?php 
...
?>
<h3>Organizer</h3>
<p><strong>Designate the Competition Organizer:</strong><span class="data">
	<select name="Organizer">
        <option value="">Choose Below:</option>
        <?php do { ?>
        <option value="<?php echo $row_brewers['uid']; ?>" <?php if (($row_brewers['uid'] == $row_organizer['uid'])) echo "SELECTED";?>><?php echo $row_brewers['brewerLastName'].", ".$row_brewers['brewerFirstName']; ?></option>
...
<thead>
 <tr>
  <th class="dataHeading bdr1B" width="1%"><input type="checkbox" onclick="toggleChecked(this.checked)" title="Check/Uncheck All"></th>
  <th class="dataHeading bdr1B" width="15%">Name</th>
  <th class="dataHeading bdr1B">Assigned As</th>
  <?php if (($filter == "judges") || ($filter == "bos")) { ?>
  <th class="dataHeading bdr1B" width="10%">ID</th>
  <th class="dataHeading bdr1B" width="15%">Rank</th>
  <?php } if (($filter == "judges") || ($filter == "stewards")) { ?>
  <th class="dataHeading bdr1B" width="25%">Location Preferences</th>
 ...
 <tr>
  ...
  <?php if ($bid == "default") { ?>
  <td class="dataList"><input name="<?php echo $staff_row_field.$row_brewer['uid']; ?>" type="checkbox" class="checkbox" value="1" <?php echo $checked; if (($filter == "staff") && ($row_organizer['uid'] == $row_brewer['uid'])) echo " DISABLED"; ?>  /></td>
  <td class="dataList"><?php echo $row_brewer['brewerLastName'].", ".$row_brewer['brewerFirstName']; ?></td>
  <td class="dataList"><?php echo $brewer_assignment; ?></td>
  <?php if (($filter == "judges") || ($filter == "bos")) { ?>
  <td class="dataList"><?php echo strtoupper($row_brewer['brewerJudgeID']); ?></td>
  <td class="dataList"><?php echo bjcp_rank($row_brewer['brewerJudgeRank'],1); if ($row_brewer['brewerJudgeMead'] == "Y") echo "<br /><em>Certified Mead Judge</em>"; ?></td>
  <?php } if (($filter == "judges") || ($filter == "stewards")) { ?>
  <td class="dataList">
  <?php 
	...
	if ($output == "") echo "None specified.";
	else echo rtrim($output,"<br>"); 
	?>
  </td>  
