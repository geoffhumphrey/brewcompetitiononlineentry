...
<table class="dataTable">
 ...
 <?php } do { 
    ...
    if ($bid != "default") { 
 	$query_judging_loc2 = sprintf("SELECT * FROM judging WHERE id='%s'", $row_brewer['brewerJudgeLocation']);
	$judging_loc2 = mysql_query($query_judging_loc2, $brewing) or die(mysql_error());
	$row_judging_loc2 = mysql_fetch_assoc($judging_loc2);
	
	$query_judging_loc3 = sprintf("SELECT * FROM judging WHERE id='%s'", $row_brewer['brewerJudgeLocation2']);
 ...
 <tr <?php echo " style=\"background-color:$color\"";?>>
  ...
  <?php if ($totalRows_stewarding2 > 1) { ?>
  <td <?php if ($filter == "judges") echo "width=\"10%\""; ?> class="dataList">
  <?php 
