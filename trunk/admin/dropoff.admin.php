...
<table class="dataTable">
 <tr>
   <?php if ((($action == "default") || ($action == "update") || ($action == "assign")) && ($section != "step4")) { ?><td class="dataList" width="5%" nowrap="nowrap"><a href="index.php?section=admin">&laquo; Back to Admin</a></td><?php } ?>
   <?php if ((($action == "add") || ($action == "edit")) && ($section != "step4")) { ?>
   <td class="dataList"><a href="index.php?section=admin&go=dropoff">&laquo;  Back to Drop-Off Location List</a></td><?php } elseif (($section != "step4") && ($filter == "default")) { ?>
   <td class="dataList"><span class="icon"><img src="images/award_star_add.png" align="absmiddle" /></span><a class="data" href="index.php?section=admin&go=dropoff&action=add">Add a Drop-Off Location</a></td><?php } ?>
   <?php if (($action == "update") || ($action == "assign")) { ?><td class="dataList"><?php if ($bid == "default") echo "Choose ".$filter." to assign.";  else echo "Check below which ".$filter." will be assigned to the ".$row_dropoff['dropLocationName']. " location."; ?></td><?php }?>
 </tr>
</table>
<?php if ((($action == "add") || ($action == "edit")) || ($section == "step4")) { ?>
...
<table class="dataTable">
 ...
 <tr <?php echo " style=\"background-color:$color\"";?>>
  ...
  <td class="dataList">
  <span class="icon"><a href="index.php?section=admin&go=<?php echo $go; ?>&action=edit&id=<?php echo $row_dropoff['id']; ?>"><img src="images/pencil.png" align="absmiddle" border="0" alt="Edit <?php echo $row_dropoff['dropLocationName']; ?>" title="Edit <?php echo $row_dropoff['dropLocationName']; ?>"></a></span><span class="icon"><a href="javascript:DelWithCon('includes/process.inc.php?section=admin&go=<?php echo $go; ?>&dbTable=drop_off&action=delete','id',<?php echo $row_dropoff['id']; ?>,'Are you sure you want to delete the <?php echo $row_dropoff['dropLocationName']; ?> location?\nThis cannot be undone.');"><img src="images/bin_closed.png" align="absmiddle" border="0" alt="Delete <?php echo $row_dropoff['dropLocationName']; ?>" title="Delete <?php echo $row_dropoff['dropLocationName']; ?>"></a></span></td>
 </tr>
