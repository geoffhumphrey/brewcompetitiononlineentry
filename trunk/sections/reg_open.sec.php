...
<?php if (!greaterDate($today,$row_contest_info['contestRegistrationDeadline'])) { ?>
	<p>Registration opened <?php echo date_convert($row_contest_info['contestRegistrationOpen'], 2); ?> and will close  <?php echo date_convert($row_contest_info['contestRegistrationDeadline'], 2); ?>. Please note: registered users will <em>not</em> be able to add, view, edit or delete entries after <?php $date = $row_contest_info['contestRegistrationDeadline']; echo date_convert($date, 2); ?>.</p>
    ...
    <?php } ?>
<?php } else { ?>
	<p>Registration for the <?php echo $row_contest_info['contestName']; ?> will open <?php echo date_convert($row_contest_info['contestRegistrationOpen'], 2); ?> and will close on <?php echo date_convert($row_contest_info['contestEntryDeadline'], 2); ?>. Please note: registered users will <em>not</em> be able to add, view, edit or delete entries after the registration close date.</p>
<?php } ?>
...
<?php } else { ?>
    <p>If you are willing to judge or steward, please return to register on or after <?php echo date_convert($row_contest_info['contestRegistrationOpen'], 2); ?>.</p>
<?php } ?>

<h2>Entries</h2>
<p>Entries will be accepted <?php echo date_convert($row_contest_info['contestEntryOpen'], 2)." through "; echo date_convert($row_contest_info['contestEntryDeadline'], 2); ?>. All entries must be received by our shipping location <?php if ($totalRows_dropoff > 0) echo "or at a drop-off location"; ?> by <?php $date = $row_contest_info['contestEntryDeadline']; echo date_convert($date, 2); ?>. Entries will not be accepted beyond this date. For details, see the <a href="index.php?section=entry">Entry Information</a> page.</p> 
