...
if ((empty($row_judging_prefs['jPrefsJudgingOpen'])) || (empty($row_judging_prefs['jPrefsJudgingClosed'])) || (empty($row_judging_prefs['jPrefsScoresheet']))) {
	...
	$alert_empty_prefs .= sprintf("<p><strong><i class=\"fa fa-exclamation-circle\"></i> %s</strong> Competition Organization preferences <strong>have not been set</strong> for the following:</p>",$label_attention);
	...
	$alert_empty_prefs .= "</div>";
	$alert_empty_prefs .= "<div>";
	$alert_empty_prefs .= sprintf("<p>Access the <a href=\"%s\">Competition Organization Preferences</a> to customize these settings. Otherwise, the default settings will be used.</p>",$base_url."index.php?section=admin&amp;go=judging_preferences");
	$alert_empty_prefs .= "</div>";
	$alert_empty_prefs .= "</div>";
