...
if (($section == "list") || ($section == "pay")) $query_log = sprintf("SELECT * FROM brewing WHERE brewBrewerID = '%s' ORDER BY brewCategorySort, brewSubCategory, brewName ASC", $row_user['id']);
elseif (($section == "brew") && ($action == "edit")) $query_log = "SELECT * FROM brewing WHERE id = '$id'";
elseif (($section == "admin") && ($go == "entries") && ($filter == "default") && ($dbTable == "default")) $query_log = "SELECT * FROM brewing ORDER BY brewCategorySort, brewSubCategory, brewName ASC";
elseif (($section == "admin") && ($go == "entries") && ($filter != "default") && ($dbTable == "default")) $query_log = "SELECT * FROM brewing WHERE brewCategorySort=$filter ORDER BY brewSubCategory, brewName ASC";
elseif (($section == "admin") && ($go == "entries") && ($filter == "default") && ($dbTable != "default")) $query_log = "SELECT * FROM $dbTable ORDER BY brewCategorySort, brewSubCategory, brewName ASC";
elseif (($section == "admin") && ($go == "entries") && ($filter != "default") && ($dbTable != "default")) $query_log = "SELECT * FROM $dbTable WHERE brewCategorySort=$filter ORDER BY brewSubCategory, brewName ASC";
else $query_log = "SELECT * FROM brewing ORDER BY brewSubCategory, brewName, brewBrewerLastName ASC";
