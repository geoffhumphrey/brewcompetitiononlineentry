...
<div id="header">
	<div id="header-inner"><h1><?php if ($action == "add") echo "Step 2: Brewer Information"; else echo "Edit Brewer Information"; ?></h1></div>
</div>
...
<form action="includes/process.inc.php?section=<?php if ($go == "default") echo "list"; else echo "admin&go=".$go."&filter=".$filter; ?>&action=<?php echo $action; ?>&dbTable=brewer&id=<?php echo $row_brewer['id']; ?>" method="POST" name="form1" id="form1" onSubmit="return CheckRequiredFields()">
<table class="dataTable">
      <tr><td class="dataLabel">First Name:</td>
      <td class="data"><input type="text" id="brewerFirstName" name="brewerFirstName" value="<?php if ($action == "edit") echo $row_brewer['brewerFirstName']; ?>" size="32" maxlength="20"></td>
...
<tr>
      ...
      <td class="data">Are you willing and qualified<br />
        to judge in this competition?</td>
      <td class="data"><input type="radio" name="brewerJudge" value="Y" id="brewerJudge_0"  <?php if (($action == "edit") && ($row_brewer['brewerJudge'] == "Y")) echo "CHECKED"; ?> /> Yes<br /><input type="radio" name="brewerJudge" value="N" id="brewerJudge_1" <?php if ($action == "add") echo "CHECKED"; if (($action == "edit") && ($row_brewer['brewerJudge'] == "N")) echo "CHECKED"; ?>/> No</td>
</tr>
<tr>
      <td class="dataLabel">Stewarding:</td>
      <td class="data">Are you willing be a steward<br />in this competition?</td>
      <td class="data"><input type="radio" name="brewerSteward" value="Y" id="brewerSteward_0"  <?php if (($action == "edit") && ($row_brewer['brewerSteward'] == "Y")) echo "CHECKED"; ?> /> Yes<br /><input type="radio" name="brewerSteward" value="N" id="brewerSteward_1" <?php if ($action == "add") echo "CHECKED"; if (($action == "edit") && ($row_brewer['brewerSteward'] == "N")) echo "CHECKED"; ?>/> No</td>
</tr>
<?php if ($action == "edit") { ?>
      <tr><td class="dataLabel">BJCP ID:</td>
      <td colspan="2" class="data"><input name="brewerJudgeID" id="brewerJudgeID" type="text" size="10" value="<?php echo $row_brewer['brewerJudgeID']; ?>" /></td>
    </tr>
<tr>
      <td class="dataLabel">BJCP Rank:</td>
      <td colspan="2" class="data"><select name="brewerJudgeRank">
        <option value ""></option>
        <option value="Experienced" <?php if ($row_brewer['brewerJudgeRank'] == "Experienced") echo "SELECTED"; ?>>Experienced</option>
        <option value="Recognized" <?php if ($row_brewer['brewerJudgeRank'] == "Recognized") echo "SELECTED"; ?>>Recognized</option>
        <option value="Certified" <?php if ($row_brewer['brewerJudgeRank'] == "Certified") echo "SELECTED"; ?>>Certified</option>
        <option value="National" <?php if ($row_brewer['brewerJudgeRank'] == "National") echo "SELECTED"; ?>>National</option>
        <option value="Master" <?php if ($row_brewer['brewerJudgeRank'] == "Master") echo "SELECTED"; ?>>Master</option>
        <option value="Grand Master" <?php if ($row_brewer['brewerJudgeRank'] == "Grand Master") echo "SELECTED"; ?>>Grand Master</option>
        <option value="Honorary Master" <?php if ($row_brewer['brewerJudgeRank'] == "Honorary Master") echo "SELECTED"; ?>>Honorary Master</option>
        <option value="Honorary Grand Master" <?php if ($row_brewer['brewerJudgeRank'] == "Honorary Grand Master") echo "SELECTED"; ?>>Honorary Grand Master</option>
      </select>      </td>
    </tr>
<tr>
