...
/**
 * Generally, "localhost" will work for most environments. However, some 
 * environments may require another hostname. This has been confirmed for 
 * GO DADDY shared hosting users. This article details how to determine your
 * database hostname for a Go Daddy enviornment.
 * https://www.godaddy.com/help/find-your-database-hostname-23889
 */
 ...
 * for your web hosting company).
 * INSERT YOUR USERNAME BETWEEN THE DOUBLE-QUOTATION MARKS ("").
 * For example, if your username is wade then the line should read 
 * $username = "wade".
 */
 ...
 * For example, if your password is wilson then the line should read 
 * $password = "wilson".
 */
 ...
 * Give your installation a unique ID. If you plan on running multiple instances
 * of BCOE&M from the same domain, you'll need to give each installation a
 * unique identifier. This prevents "cross-pollination" of session data.
 *
 * For single installations, the default below will be sufficient. Otherwise,
 * change the variable to something unique for each installation.
 */
 ...
 *
 * If you are installing on a server where you do not have a domain name set up,
 * you'll need to replace the last $base_url variable line below with something
 * formatted like this:
 * $base_url .= "yourhostingdomain/~accountname/subdirectoryname/";
 *
 * Example:
 ...
 * $base_url .= "www.bluehost.com/~brewcompeition/bcoem/";
 *
 * --- Problems logging in after setup? On a local machine? ---
 * If you are expreriencing issues logging in, if forms are not posting 
 * information, or if you are installing on a local computer (e.g., a localhost),
 * you may need to change all URLs generated by the application to be relative 
 * instead of absolute. To do so, comment out the first three $base_url variable 
 * lines below and add uncomment the last $base_url variable line to ensure that
 * all functions work as expected.
 *
 * Absolute URLs are preferred and the default will work in most environments.
 * Use only if experiencing issues.
 *
 * --- No Secure Socket Layer (SSL)? ---
 * To override the SSL (HTTPS) check if SSL isn't implemented on your
 * server AND you're experiencing log in or session issues, or if pages are not 
 * rendering correctly, comment out the second line in the block below (the if
 * statement).
