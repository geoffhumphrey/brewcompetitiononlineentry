...
// Map BJCP2008 Styles to BJCP2015 Styles and vice/versa

function bjcp_convert($styleSet) {
	// Param: $styleSet: Get newly requested style set (converting TO)
	
	include(CONFIG.'config.php');
	mysql_select_db($database, $brewing);
	
	
	$query_brews = sprintf("SELECT id,brewCategory,brewCategorySort,brewSubCategory,brewStyle FROM %s ORDER BY brewCategorySort,brewSubCategory", $prefix."brewing");
	$brews = mysql_query($query_brews, $brewing) or die(mysql_error());
	$row_brews = mysql_fetch_assoc($brews);
	
	// Convert from BJCP2008 to BJCP2015
	if ($styleSet == "BJCP2015") {
	...
	// Convert from BJCP2015 to BJCP2008
	if ($styleSet == "BJCP2008") {
