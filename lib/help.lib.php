...
function bcoem_dashboard_help($content) {
	...
	switch($content) {
		...
		$bcoem_dashboard_help_body .= "<p>BCOE&amp;M offers several ways to customize the behavior of your BCOE&amp;M installation. Preferences are divided into seven salient categories:</p>";
		$bcoem_dashboard_help_body .= "<ol>";
		$bcoem_dashboard_help_body .= "<li><a href=\"".$base_url."index.php?section=admin&amp;go=preferences\">General Preferences</a> include the competition type (amateur or professional), admin theme, results date and time display, winner distribution method (by medal group, by style, or by sub-style), CAPTCHA, and localization.</li>";
		$bcoem_dashboard_help_body .= "<li><a href=\"".$base_url."index.php?section=admin&amp;go=preferences&amp;action=entries\">Entry Preferences</a> include style set, entry label options, entry fees and discounts, and any overall entry limits, per-style type and per-style limits.</li>";
		$bcoem_dashboard_help_body .= "<li><a href=\"".$base_url."index.php?section=admin&amp;go=hero_images\">Banner Image Preferences</a> allow Admins to choose the banner images shown on the homepage and/or upload custom images that meet minimum criteria.</li>";
		$bcoem_dashboard_help_body .= "<li><a href=\"".$base_url."index.php?section=admin&amp;go=preferences&amp;action=email\">Email Sending / Contact Display Preferences</a> are those that govern whether or not the BCOE&amp;M system can send emails. To enable the system to send emails, you will need to provide a valid email address and associated information to send emails via the Simple Mail Transfer Protocol (SMTP).</li>";
		$bcoem_dashboard_help_body .= "<li><a href=\"".$base_url."index.php?section=admin&amp;go=preferences&amp;action=payment\">Currency and Payment Preferences</a> allow for defining of accepted currency and the methods the competition will employ to collect fees (cash, check, PayPal).</li>";
		...
		$bcoem_dashboard_help_body .= "<li><a href=\"".$base_url."index.php?section=admin&amp;go=judging_preferences\">Judging/Competition Organization Preferences</a> are those affect how BCOE&amp;M behaves with regard to tables, flights, and best of show functions.</li>";
		$bcoem_dashboard_help_body .= "<ol>";
		break;
