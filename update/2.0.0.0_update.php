...
if (!check_update("sponsorEnable", $prefix."sponsors")) {
	...
	if ($db_conn->getLastErrno() === 0) $output .=  "<li>Sponsors table altered successfully.</li>";
	else $output .= "<li class=\"text-danger\">Error: Sponsors table NOT altered. ".$db_conn->getLastError()."</li>";
}
...
if ($block_ok) $output .= "<li>Competition info table altered successfully.</li>";
...
$rows_archive_current = $db_conn->rawQuery($query_archive_current);
$totalRows_archive_current = count($rows_archive_current);
...
if ($totalRows_archive_current > 0) {
	...
	foreach ($a_current as $suffix_current) {
		...
		$suffix_current = preg_replace("/[^a-zA-Z0-9]+/", "", $suffix_current);
...
if ($block_ok) $output .= "<li>All archive brewer tables updated successfully.</li>";
...
if ($db_conn->getLastErrno() === 0) $output .=  "<li>Countries table removed from the database.</li>";
else $output .= "<li class=\"text-danger\">Error: Countries table NOT removed. ".$db_conn->getLastError()."</li>";
...
if ($db_conn->getLastErrno() === 0) $output .=  "<li>Themes table removed from the database.</li>";
else $output .= "<li class=\"text-danger\">Error: Themes table NOT removed. ".$db_conn->getLastError()."</li>";
...
if ($block_ok) $output .= "<li>Sponsor data updated.</li>";
...
if ($block_ok) $output .= "<li>Style data updated.</li>";
