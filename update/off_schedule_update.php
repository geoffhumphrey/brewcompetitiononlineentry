...
if ((!check_update("setup_last_step", $prefix."system")) || (FORCE_UPDATE)) {
	...
	if (!check_update("setup_last_step", $prefix."system")) {
		...
		// Add new specialty IPA and historical styles to styles table
...
if ((!check_update("brewerBreweryName", $prefix."brewer")) || (FORCE_UPDATE)) {
	...
	$updateSQL = sprintf("ALTER TABLE `%s` CHANGE `contestEntryFee` `contestEntryFee` FLOAT(6,2) NULL DEFAULT NULL, CHANGE `contestEntryFee2` `contestEntryFee2` FLOAT(6,2) NULL DEFAULT NULL;",$prefix."contest_info");
	mysqli_select_db($connection,$database);
	mysqli_real_escape_string($connection,$updateSQL);
	$result = mysqli_query($connection,$updateSQL);
