...
<?php if (($_SESSION['contestLogo'] != "") && (file_exists($_SERVER['DOCUMENT_ROOT'].$sub_directory.'/user_images/'.$_SESSION['contestLogo']))) { // display competition's logo if name present in DB and in the correct folder on the server ?>
...
<?php if ($row_limits['prefsEntryLimit'] != "") { ?><a href="#entry_limit">Entry Limit</a><br /><?php } ?>
...
<?php if ($_SESSION['contestBottles'] != "") { ?><a href="#bottle">Bottle Acceptance Rules</a><br /><?php } ?>
<?php if ($_SESSION['contestShippingAddress'] != "") { ?><a href="#shipping">Shipping Location and Address</a><br /><?php } ?>
<?php if ($totalRows_dropoff > 0) { ?><a href="#drop">Drop Off Location<?php if ($totalRows_dropoff > 1) echo "s"; ?></a><br /><?php } ?>
<?php if ($_SESSION['contestBOSAward'] != "") { ?><a href="#bos">Best of Show</a><br /><?php }?>
<?php if ($_SESSION['contestAwards'] != "") { ?><a href="#awards">Awards</a><br /><?php }?>
<?php if ($_SESSION['contestAwardsLocName'] != "") { ?><a href="#ceremony">Awards Ceremony</a><br /><?php } ?>
<?php if ($_SESSION['contestCircuit'] != "") { ?><a href="#circuit">Circuit Qualification</a><?php } ?>
<?php } ?>
...
<p><?php echo $_SESSION['prefsCurrency'].number_format($_SESSION['contestEntryFee'],2); ?> per entry. <?php if ($_SESSION['contestEntryFeeDiscount'] == "Y") echo $_SESSION['prefsCurrency'].number_format($_SESSION['contestEntryFee2'], 2)." per entry after the ".addOrdinalNumberSuffix($_SESSION['contestEntryFeeDiscountNum'])." entry. "; if ($_SESSION['contestEntryCap'] != "") echo $_SESSION['prefsCurrency'].number_format($_SESSION['contestEntryCap'], 2)." for unlimited entries. "; if (NHC) echo $_SESSION['prefsCurrency'].number_format($_SESSION['contestEntryFeePasswordNum'],2)." for AHA members."; ?></p>
<?php if ($row_limits['prefsEntryLimit'] != "") { ?>
...
<p>After registering and inputting entries, all participants must pay their entry fee(s).</p>
    <ul>
    	<?php 
		if ($_SESSION['prefsCash'] == "Y") echo "<li>Cash</li>";
        if ($_SESSION['prefsCheck'] == "Y") echo "<li>Check, made out to <em>".$_SESSION['prefsCheckPayee']."</em></li>";
        if ($_SESSION['prefsPaypal'] == "Y") echo "<li>PayPal</li>";
		if ($_SESSION['prefsGoogle'] == "Y") echo "<li>Google Wallet</li>"; 
		?>
    </ul>
...
<?php if ($_SESSION['contestBottles'] != "") { ?>
<a name="bottle"></a><h2>Bottle Acceptance Rules</h2>
<?php echo $_SESSION['contestBottles']; ?>
<?php } if ($_SESSION['contestShippingAddress'] != "") { ?>
<a name="shipping"></a><h2>Shipping Location and Address</h2>
<p><?php echo $_SESSION['contestShippingName']."<br>".$_SESSION['contestShippingAddress']; ?></p>
...
<p>Enclose your <em>each</em> of your bottle labels in a small zip-top bag before attaching to their respective bottles. This way it makes it possible for the organizer to identify specifically  which beer has broken if there is damage during shipment.</p>
...
<?php } ?>
<?php if ($_SESSION['contestBOSAward'] != "") { ?>
<a name="bos"></a><h2>Best of Show</h2>
<?php echo $_SESSION['contestBOSAward']; ?>
<?php }
if ($_SESSION['contestAwards'] != "") { ?>
<a name="awards"></a><h2>Awards</h2>
<?php echo $_SESSION['contestAwards']; ?>
<?php } ?>
<?php if ($_SESSION['contestAwardsLocName'] != "") { ?>
<a name="ceremony"></a><h2>Award Ceremony</h2>
<p>
	<?php 
	 echo "<strong>".$_SESSION['contestAwardsLocName']."</strong>";
     if ($_SESSION['contestAwardsLocation'] != "") echo "<br />".$_SESSION['contestAwardsLocation']; 
	 if (($_SESSION['contestAwardsLocation'] != "") && ($action != "print")) { ?>&nbsp;&nbsp;<span class="icon"><a id="modal_window_link" href="<?php echo $base_url; ?>output/maps.php?section=map&amp;id=<?php echo str_replace(' ', '+', $_SESSION['contestAwardsLocation']); ?>" title="Map to <?php echo $_SESSION['contestAwardsLocName']; ?>"><img src="<?php echo $base_url; ?>images/map.png"  border="0" alt="Map <?php echo $_SESSION['contestAwardsLocName']; ?>" title="Map <?php echo $_SESSION['contestAwardsLocName']; ?>" /></a></span>
	<span class="icon"><a href="<?php echo $base_url; ?>output/maps.php?section=driving&amp;id=<?php echo str_replace(' ', '+', $_SESSION['contestAwardsLocation']); ?>" title="Driving Directions to <?php echo $_SESSION['contestAwardsLocName']; ?>" target="_blank"><img src="<?php echo $base_url; ?>images/car.png"  border="0" alt="Driving Directions to <?php echo $_SESSION['contestAwardsLocName']; ?>" title="Driving Direcitons to <?php echo $_SESSION['contestAwardsLocName']; ?>" /></a></span>
	<?php } 
	if ($_SESSION['contestAwardsLocTime'] != "") echo "<br />". 
	getTimeZoneDateTime($_SESSION['prefsTimeZone'], $_SESSION['contestAwardsLocTime'], $_SESSION['prefsDateFormat'],  $_SESSION['prefsTimeFormat'], "long", "date-time");
	?>
</p>
<?php } 

if ($_SESSION['contestCircuit'] != "") { ?>
<a name="circuit"></a><h2>Circuit Qualification</h2>
<?php 
