...
if (($session_active) && ($_SESSION['userLevel'] <= 2) && ($referrer_ok)) {
	...
	if ($action == "evaluation") {
		...
		if ($go == "evalPlace") {
			...
			if (empty($input)) $data = [$go => NULL];
			else {
				if ($input == "0") $data = [$go => NULL];			
				else $data = [$go => $input];
			}
		}
...
if (($session_active) && ($_SESSION['userLevel'] <= 1) && ($referrer_ok)) {
	...
	if ($action == "brewing") {
		...
		if (empty($input)) {
			...
			if ($rid2 == "text-col") {
				$data = [$go => '', 'brewUpdated' => date('Y-m-d H:i:s', time())];
			}

			else {
				$data = [$go => NULL, 'brewUpdated' => date('Y-m-d H:i:s', time())];
			}
			...
			if ($input == "0") {
				...
				$data = [$go => $input, 'brewUpdated' => date('Y-m-d H:i:s', time())];
			}
