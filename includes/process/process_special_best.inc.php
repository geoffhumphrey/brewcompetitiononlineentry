...
if ($action == "add") {
	$insertSQL = sprintf("INSERT INTO special_best_info (sbi_name, sbi_description, sbi_places, sbi_rank) VALUES (%s, %s, %s, %s)",
...
if ($action == "edit") {
	$updateSQL = sprintf("UPDATE special_best_info SET sbi_name=%s, sbi_description=%s, sbi_places=%s, sbi_rank=%s WHERE id=%s",
					   GetSQLValueString(capitalize($_POST['sbi_name']), "text"),
					   GetSQLValueString($_POST['sbi_description'], "text"),
					   GetSQLValueString($_POST['sbi_places'], "int"),
					   GetSQLValueString($_POST['sbi_rank'], "int"),
