...
$query_bos_scores = sprintf("SELECT a.scorePlace, a.scoreEntry, c.uid FROM %s a, %s b, %s c WHERE a.eid = b.id AND c.uid = a.bid AND a.scorePlace IS NOT NULL ", $judging_scores_bos_db_table, $brewing_db_table, $brewer_db_table);
...
$query_bb_prefs = sprintf("SELECT prefsBestBrewerTitle, prefsFirstPlacePts, prefsSecondPlacePts, prefsThirdPlacePts, prefsFourthPlacePts, prefsHMPts, prefsTieBreakRule1, prefsTieBreakRule2, prefsTieBreakRule3, prefsTieBreakRule4, prefsTieBreakRule5, prefsTieBreakRule6 FROM %s WHERE id='1'", $prefix."preferences");
