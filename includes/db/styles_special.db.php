...
foreach ($rows_required_optional as $row_required_optional) {
	...
	if ($section == "brew") {
		...
		if (($row_required_optional['brewStyleReqSpec'] == 1) || ($row_required_optional['brewStyleStrength'] == 1) || ($row_required_optional['brewStyleSweet'] == 1) || ($row_required_optional['brewStyleCarb'] == 1)) {
			...
			if (!$_SESSION['style_set_no_numbering']) $info .= str_replace($replacement1,$replacement2,"<p>".$row_required_optional['brewStyleInfo']."</p>");
			...
			$add_edit_entry_modals .= "\t\t\t\t<button type=\"button\" class=\"btn-close\" data-bs-dismiss=\"modal\" aria-label=\"Close\"></button>\n";
			$add_edit_entry_modals .= "\t\t\t</div>\n";
			$add_edit_entry_modals .= "\t\t\t<div class=\"modal-body\">".$info."</div>\n";
			$add_edit_entry_modals .= "\t\t\t<div class=\"modal-footer\">\n";
			$add_edit_entry_modals .= "\t\t\t\t<button type=\"button\" class=\"btn btn-danger\" data-bs-dismiss=\"modal\">Close</button>\n";
			$add_edit_entry_modals .= "\t\t\t</div>\n";
			$add_edit_entry_modals .= "\t\t</div>\n";
			$add_edit_entry_modals .= "\t</div>\n";
			$add_edit_entry_modals .= "</div>\n\n";
