...
if (table_exists($style_types_db_table)) {
	...
	if (($go == "default") || ($go == "style_types") || ($go == "styles") || ($go == "judging_scores")  || ($go == "judging_scores_bos") || ($go == "judging_tables") || ($go == "judging") || ($go == "staff") || ($go == "preferences") || ($go == "csv")) {
		...
		$rows_style_type = (!empty($params_style_type)) ? $db_conn->rawQuery($sql, $params_style_type) : $db_conn->rawQuery($sql);
...
if (table_exists($judging_tables_db_table)) {
	...
	if (($go == "default") || ($go == "participants") || ($go == "judging_scores") || ($go == "judging_tables") || ($go == "judging_flights") || ($go == "judging_tables") || ($go == "judging_locations")) {
		...
		if (($section == "table_cards") && ($go == "judging_locations")) {
			...
			$params_tables = array($location, $round);
		}

		$sql_edit = "SELECT * FROM ".$judging_tables_db_table;
		$params_tables_edit = array();
		...
		$rows_tables = (!empty($params_tables)) ? $db_conn->rawQuery($sql, $params_tables) : $db_conn->rawQuery($sql);
		$row_tables = ($rows_tables && count($rows_tables) > 0) ? $rows_tables[0] : null;
		$totalRows_tables = $db_conn->count;

		$rows_tables_edit = (!empty($params_tables_edit)) ? $db_conn->rawQuery($sql_edit, $params_tables_edit) : $db_conn->rawQuery($sql_edit);
		$row_tables_edit = ($rows_tables_edit && count($rows_tables_edit) > 0) ? $rows_tables_edit[0] : null;
		$totalRows_tables_edit = $db_conn->count;

		$rows_tables_edit_2 = (!empty($params_tables_edit)) ? $db_conn->rawQuery($sql_edit, $params_tables_edit) : $db_conn->rawQuery($sql_edit);
...
if (check_setup($judging_scores_db_table,$database)) {
	...
	$rows_scores = (!empty($params_scores)) ? $db_conn->rawQuery($sql, $params_scores) : $db_conn->rawQuery($sql);
...
if (($go == "judging_scores_bos") || ($go == "judging_tables") || ($go == "output") || ($go == "default")) {
	...
	$sql = "SELECT * FROM ".$style_types_db_table;
	$params_style_types = array();
	if (SINGLE) { $sql .= " WHERE comp_id='0' OR comp_id=?"; $params_style_types[] = $_SESSION['comp_id']; }
	$rows_style_types = (!empty($params_style_types)) ? $db_conn->rawQuery($sql, $params_style_types) : $db_conn->rawQuery($sql);
...
if (($go == "default") || ($go == "participants")) {
	...
	$params_with_entries = array();
	if (SINGLE) { $sql .= " WHERE comp_id = ?"; $params_with_entries[] = $_SESSION['comp_id']; }
	$rows_with_entries = (!empty($params_with_entries)) ? $db_conn->rawQuery($sql, $params_with_entries) : $db_conn->rawQuery($sql);
...
if (($go == "special_best_data") || ($go == "special_best")) {
	...
	if (SINGLE) {
		...
		$params_sbi = array($_SESSION['comp_id']);
		...
		if ($action == "add") { $sql_sbd = "SELECT * FROM ".$special_best_data_db_table." WHERE comp_id=? AND id=?"; $params_sbd = array($_SESSION['comp_id'], $id); }
		elseif ($action == "edit") { $sql_sbd = "SELECT * FROM ".$special_best_data_db_table." WHERE comp_id=? AND sid=? ORDER BY sbd_place ASC"; $params_sbd = array($_SESSION['comp_id'], $id); }
		else { $sql_sbd = "SELECT * FROM ".$special_best_data_db_table." WHERE comp_id=? ORDER BY sid,sbd_place ASC"; $params_sbd = array($_SESSION['comp_id']); }
		...
		$rows_sbi = (!empty($params_sbi)) ? $db_conn->rawQuery($sql_sbi, $params_sbi) : $db_conn->rawQuery($sql_sbi);
		$row_sbi = ($rows_sbi && count($rows_sbi) > 0) ? $rows_sbi[0] : null;
		$totalRows_sbi = $db_conn->count;

		if ($action == "add") { $sql_sbd = "SELECT * FROM ".$special_best_data_db_table." WHERE id=?"; $params_sbd = array($id); }
		elseif ($action == "edit") { $sql_sbd = "SELECT * FROM ".$special_best_data_db_table." WHERE sid=? ORDER BY sbd_place ASC"; $params_sbd = array($id); }
		else { $sql_sbd = "SELECT * FROM ".$special_best_data_db_table." ORDER BY sid,sbd_place ASC"; $params_sbd = array(); }
		$rows_sbd = (!empty($params_sbd)) ? $db_conn->rawQuery($sql_sbd, $params_sbd) : $db_conn->rawQuery($sql_sbd);
		$row_sbd = ($rows_sbd && count($rows_sbd) > 0) ? $rows_sbd[0] : null;
		$totalRows_sbd = $db_conn->count;
