...
if (table_exists($style_types_db_table)) {
	...
	if (($go == "default") || ($go == "style_types") || ($go == "styles") || ($go == "judging_scores")  || ($go == "judging_scores_bos") || ($go == "judging_tables") || ($go == "judging")) {
		...
		if (SINGLE) {
			$query_style_type = sprintf("SELECT * FROM $style_types_db_table WHERE comp_id='%s'",$_SESSION['comp_id']); 
...
if (($go == "judging_scores_bos") || ($go == "judging_tables") || ($go == "output")) {
	...
	if (SINGLE) $query_style_types .= sprintf(" WHERE comp_id='%s'",$_SESSION['comp_id']); 
	...
	if ($action == "default") {
		...
		if (SINGLE) $query_style_types_2 .= sprintf(" WHERE comp_id='%s'",$_SESSION['comp_id']); 
