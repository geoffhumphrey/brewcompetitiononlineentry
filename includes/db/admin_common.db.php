...
if (table_exists($style_types_db_table)) {
	...
	if (($go == "default") || ($go == "style_types") || ($go == "styles") || ($go == "judging_scores")  || ($go == "judging_scores_bos") || ($go == "judging_tables") || ($go == "judging") || ($go == "staff") || ($go == "preferences") || ($go == "csv")) {
		...
		if (($action == "edit") && ($filter != "default")) {
			$db_conn->where ('id', $filter);
			$style_types_single_result = TRUE;
		}

		if (($action == "enter") && ($filter != "default")) {
			$db_conn->where ('id', $filter);
			$style_types_single_result = TRUE;
		}

		if (($go != "styles") && ($id != "default")) {
			$db_conn->where ('id', $id);
			$style_types_single_result = TRUE;
		}
		...
		$result_style_types = $db_conn->get($style_types_db_table);
...
if (table_exists($judging_tables_db_table)) {
	...
	if (($go == "default") || ($go == "participants") || ($go == "judging_scores") || ($go == "judging_tables") || ($go == "judging_flights") || ($go == "judging_tables") || ($go == "judging_locations")) {
		...
		/*
		// Single Result Flag
		$judging_tables_single_result = FALSE;

		// Join query. Use RawSQL.
		if (($section == "table_cards") && ($go == "judging_locations")) {

			$sql = sprintf("SELECT a.*, b.assignRound FROM $judging_tables_db_table a, $judging_assignments_db_table b WHERE a.id = b.assignTable AND a.tableLocation = '%s' AND b.assignRound = '%s' GROUP BY b.assignTable ORDER BY tableNumber", $location, $round);
			$result_tables = $db_conn->rawQuery($sql);
			...
			if (($go == "judging_tables") || ($go == "judging_scores") || (($section == "table_cards") && ($go == "judging_tables"))) {
				$db_conn->orderBy ('tableNumber', 'ASC');
			}

			$db_conn->returnType = 'array'; 
			$result_tables = $db_conn->get($judging_tables_db_table);
