...
if ($filter == "mini_bos") { 
	...
	if (strpos($_SESSION['prefsStyleSet'],"BABDB") !== false) $query_entries = sprintf("SELECT a.scoreMiniBOS, b.id, b.brewStyle, b.brewCategory, b.brewCategorySort, b.brewSubCategory, b.brewInfo, b.brewMead1, b.brewMead2, b.brewMead3, b.brewJudgingNumber, b.brewBoxNum, b.brewComments FROM %s a, %s b WHERE b.brewSubCategory='%s' AND a.eid = b.id AND a.scoreMiniBOS='1' ORDER BY %s", $judging_scores_db_table, $brewing_db_table, $value, $order);
	
	else $query_entries = sprintf("SELECT a.scoreMiniBOS, b.id, b.brewStyle, b.brewCategory, b.brewCategorySort, b.brewSubCategory, b.brewInfo, b.brewMead1, b.brewMead2, b.brewMead3, b.brewJudgingNumber, b.brewBoxNum, b.brewComments FROM %s a, %s b WHERE b.brewCategorySort='%s' AND b.brewSubCategory='%s' AND a.eid = b.id AND a.scoreMiniBOS='1' ORDER BY %s", $judging_scores_db_table, $brewing_db_table, $row_styles['brewStyleGroup'], $row_styles['brewStyleNum'], $order);
