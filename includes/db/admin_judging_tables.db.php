...
if (SINGLE) $query_table_number = sprintf("SELECT tableNumber FROM $judging_tables_db_table ORDER BY tableNumber WHERE comp_id='%s'",$_SESSION['comp_id']); 
else $query_table_number = "SELECT tableNumber FROM $judging_tables_db_table ORDER BY tableNumber";
...
if ($action == "add") {
	...
	if (SINGLE) $query_table_number_last = sprintf("SELECT tableNumber FROM $judging_tables_db_table WHERE comp_id='%s' ORDER BY tableNumber DESC LIMIT 1",$_SESSION['comp_id']); 
	else $query_table_number_last = "SELECT tableNumber FROM $judging_tables_db_table ORDER BY tableNumber DESC LIMIT 1";
...
if (SINGLE) {
	$query_entry_count = sprintf("SELECT COUNT(*) as 'count' FROM $brewing_db_table WHERE comp_id='%s'",$_SESSION['comp_id']);
