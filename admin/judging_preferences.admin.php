...
if (($_SESSION['prefsEval'] == 1) || ($section == "step8")) {
    ...
    // The date-time-picker-system fields below (Tempus Dominus) always display/accept
    // 24-hour time regardless of the competition's own prefsTimeFormat preference - see
    // js_source/app.js's Tempus Dominus init comment for why. Pass "1" (24-hour) here
    // instead of $_SESSION['prefsTimeFormat'] so this field's initial value always matches
    // what the picker itself expects, independent of that preference.
    if (!isset($_SESSION['jPrefsJudgingOpen'])) {

        if (empty($judging_earliest_date)) $judging_open_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], time(), $_SESSION['prefsDateFormat'],  "1", "system", "date-time-system");
        else $judging_open_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $judging_earliest_date, $_SESSION['prefsDateFormat'],  "1", "system", "date-time-system");

    }

    if (!isset($_SESSION['jPrefsJudgingClosed'])) {

        if (empty($judging_latest_date)) $judging_close_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], (time()+86400), $_SESSION['prefsDateFormat'],  "1", "system", "date-time-system"); // Close one day after today
        else $judging_close_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $judging_latest_date, $_SESSION['prefsDateFormat'],  "1", "system", "date-time-system");
    ...
    if ((isset($_SESSION['jPrefsJudgingOpen'])) && (!empty($_SESSION['jPrefsJudgingOpen']))) {
        ...
        $judging_open_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $j_open_date, $_SESSION['prefsDateFormat'],  "1", "system", "date-time-system");
        ...
        $judging_open_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $suggested_open_date, $_SESSION['prefsDateFormat'],  "1", "system", "date-time-system");
        $suggested_open = TRUE;
    ...
    if ((isset($_SESSION['jPrefsJudgingClosed'])) && (!empty($_SESSION['jPrefsJudgingClosed']))) {
        ...
        $judging_close_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $j_closed_date, $_SESSION['prefsDateFormat'],  "1", "system", "date-time-system");
        ...
        $judging_close_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $suggested_close_date, $_SESSION['prefsDateFormat'],  "1", "system", "date-time-system");
...
<form class="form-horizontal hide-loader-form-submit needs-validation" role="form" method="post" action="<?php echo $base_url; ?>includes/process.inc.php?section=<?php if ($section == "step8") echo "setup"; else echo $section; ?>&amp;action=edit&amp;dbTable=<?php echo $judging_preferences_db_table; ?>&amp;id=1" name="form1" novalidate>
...
<?php } ?>
<div class="row mb-3">
    <label for="jPrefsBottleNum" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Number of Bottles Required per Entry</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
            <select class="form-select bootstrap-select" name="jPrefsBottleNum" id="jPrefsBottleNum">
            <?php for ($i=1; $i <= 15; $i++) { ?>
            ...
            <div id="helpBlock" class="help-block">Most competitions require at least two bottles.</div>
    </div>
</div>
<div class="row mb-3"><!-- Form Group Radio INLINE -->
    <label for="jPrefsQueued" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Use Queued Judging</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="jPrefsQueued" value="Y" id="jPrefsQueued_0" rel="none" <?php if ((isset($_SESSION['jPrefsQueued'])) && ($_SESSION['jPrefsQueued'] == "Y")) echo "checked"; elseif ($section == "step8") echo "checked"; ?>>
            <label class="form-check-label" for="jPrefsQueued_0">Yes</label>
        </div>
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="jPrefsQueued" value="N" id="jPrefsQueued_1" rel="queued_no" <?php if ((isset($_SESSION['jPrefsQueued'])) && ($_SESSION['jPrefsQueued'] == "N")) echo "checked"; ?>>
            <label class="form-check-label" for="jPrefsQueued_1">No</label>
        </div>
        <div class="help-block">
		<div class="btn-group" role="group" aria-label="queuedModal">
			<div class="btn-group" role="group">
				<button type="button" class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#queuedModal">
				   Queued Judging Info
				</button>
			</div>
		</div>
...
<div class="row mb-3">
    <label for="prefsDisplaySpecial" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Scoresheet Unique Identifier</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="prefsDisplaySpecial" value="J" id="prefsDisplaySpecial_0" <?php if (($section == "step3") || ($_SESSION['prefsDisplaySpecial'] == "J")) echo "checked"; ?>>
            <label class="form-check-label" for="prefsDisplaySpecial_0">6-Character Judging Number</label>
        </div>
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="prefsDisplaySpecial" value="E" id="prefsDisplaySpecial_1" <?php if ($_SESSION['prefsDisplaySpecial'] == "E") echo "checked"; ?>>
            <label class="form-check-label" for="prefsDisplaySpecial_1">6-Digit Entry Number</label>
        </div>
...
<div class="row mb-3">
    <label for="prefsEval" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Electronic Scoresheets</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="prefsEval" value="1" id="prefsEval_1" <?php if ($_SESSION['prefsEval'] == "1") echo "checked"; elseif ($section == "step3") echo "checked"; ?>>
            <label class="form-check-label" for="prefsEval_1">Enable</label>
        </div>
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="prefsEval" value="0" id="prefsEval_0" <?php if ($_SESSION['prefsEval'] == "0") echo "checked"; ?>>
            <label class="form-check-label" for="prefsEval_0">Disable</label>
        </div>
        <div class="help-block">
        <div class="btn-group" role="group" aria-label="prefsEvalModal">
            <div class="btn-group" role="group">
                <button type="button" class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#prefsEvalModal">
...
<div class="row mb-3"><!-- Form Group Radio INLINE -->
    <label for="jPrefsScoresheet" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Entry Evaluation Scoresheet</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
        <div class="form-check">
            <input class="form-check-input" type="radio" name="jPrefsScoresheet" value="1" id="jPrefsScoresheet_0" rel="" <?php if ($judging_scoresheet == "1") echo "checked"; elseif ($section == "step8") echo "checked"; ?>>
            <label class="form-check-label" for="jPrefsScoresheet_0">BJCP Classic Scoresheet (All Style Types - Including Custom)</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="jPrefsScoresheet" value="2" id="jPrefsScoresheet_1" rel="" <?php if ($judging_scoresheet == "2") echo "checked"; ?>>
            <label class="form-check-label" for="jPrefsScoresheet_1">BJCP Checklist Scoresheet (Beer Only)</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="jPrefsScoresheet" value="3" id="jPrefsScoresheet_2" rel="" <?php if ($judging_scoresheet == "3") echo "checked"; ?>>
            <label class="form-check-label" for="jPrefsScoresheet_2">BJCP Structured Scoresheet (Beer, Mead, and Cider Only)</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="jPrefsScoresheet" value="4" id="jPrefsScoresheet_3" rel="" <?php if ($judging_scoresheet == "4") echo "checked"; ?>>
            <label class="form-check-label" for="jPrefsScoresheet_3">NW Cider Cup Structured Scoresheet (Cider Only)</label>
        </div>
        <div class="help-block">
        <div class="btn-group" role="group" aria-label="queuedModal">
            <div class="btn-group" role="group">
                <button type="button" class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#scoresheetModal">
...
<div class="row mb-3">
    <label for="jPrefsMinWords" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Minimum Words for Scoresheet Comment/Feedback Fields</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
        ...
        <div id="helpBlock" class="help-block"><p>The minimum number of words judges must use when providing comments. Will be enforced for <strong>all comment fields</strong> on the Classic Scoresheet and Checklist Scoresheet, as well as the <strong>Overall Impression comment/feedback field</strong> on all Structured Scoresheets.</p><p>Leave blank or enter zero for no enforced minimum.</p></div>
    </div>
</div>
<div class="row mb-3">
    <label for="jPrefsScoreDispMax" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Maximum Difference for Consensus Scores</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
        <select class="form-select bootstrap-select" name="jPrefsScoreDispMax" id="jPrefsScoreDispMax">
        <?php for ($i=1; $i <= 10; $i++) { ?>
        ...
        <div id="helpBlock" class="help-block">
            ...
            <div class="btn-group" role="group" aria-label="maxDiffModal">
                <div class="btn-group" role="group">
                    <button type="button" class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#maxDiffModal">
...
<div class="row mb-3">
    <label for="jPrefsJudgingOpen" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Judging Open Date and Time</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
...
<div class="row mb-3">
    <label for="jPrefsJudgingClosed" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Judging Close Date and Time</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
...
<div class="row mb-3">
    <label for="jPrefsCapJudges" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Judge Limit</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
        ...
        <div id="helpBlock" class="help-block">Limit to the number of judges that may sign up. Leave blank for no limit.</div>
    </div>
</div>
<div class="row mb-3">
    <label for="jPrefsCapStewards" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Steward Limit</label>
    <div class="col-xs-12 col-sm-8 col-lg-6">
        ...
        <div id="helpBlock" class="help-block">Limit to the number of stewards that may sign up. Leave blank for no limit.</div>
    </div>
</div>
<div id="queued_no">
	<div class="row mb-3">
		<label for="jPrefsFlightEntries" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Maximum Entries per Flight</label>
		<div class="col-xs-12 col-sm-8 col-lg-6">
                <select class="form-select bootstrap-select" name="jPrefsFlightEntries" id="jPrefsFlightEntries">
                <?php for ($i=1; $i <= 50; $i++) { ?>
            ...
            <div id="helpBlock" class="help-block">The maximum number of entries a judge pair will be assigned to evaluate in the system per flight. This generally applies to the traditional (non-queued) judging methodology.</div>
		</div>
	</div>
</div>
<div class="row mb-3">
	<label for="jPrefsRounds" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Maximum Rounds per Session</label>
	<div class="col-xs-12 col-sm-8 col-lg-6">
        <select class="form-select bootstrap-select" name="jPrefsRounds" id="jPrefsRounds">
        <?php for ($i=1; $i <= 5; $i++) { ?>
        ...
        <div id="helpBlock" class="help-block">The maximum number of judging rounds for each <a href="<?php echo $base_url; ?>index.php?section=admin&amp;go=judging">defined judging session</a>.</div>
	</div>
</div>
<div class="row mb-3">
	<label for="jPrefsMaxBOS" class="col-xs-12 col-sm-4 col-lg-2 col-form-label">Maximum Places in BOS Round</label>
	<div class="col-xs-12 col-sm-8 col-lg-6">
        <select class="form-select bootstrap-select" name="jPrefsMaxBOS" id="jPrefsMaxBOS">
            <?php for ($i=1; $i <= 4; $i++) { ?>
