...
<div class="bcoem-admin-element hidden-print">
    ...
    <div class="btn-group" role="group" aria-label="markEntriesAs">
        <div class="btn-group" role="group">
            ...
            <ul class="dropdown-menu">
                <li class="small"><a href="<?php echo $base_url; ?>includes/process.inc.php?action=paid&amp;dbTable=<?php echo $brewing_db_table; ?>" data-confirm="Are you sure? This will mark ALL entries as paid and could be a large pain to undo.">Mark all Entries As Paid</a></li>
...
<thead>
    <tr>
        <th nowrap>Entry</th>
        <th nowrap>Judging <a href="#" role="button" data-toggle="tooltip" data-container="body" title="<?php echo "Judging numbers are automatically assigned by the system."; if ($_SESSION['prefsEntryForm'] == "N") echo " You can override each judging number when scanning in barcodes or by entering it in the field provided."; ?>"><span class="hidden-xs hidden-sm hidden-md hidden-print fa fa-question-circle"></span></a></th>
...
<tbody>
 <?php 
	...
	if (($row_log['brewConfirmed'] == 0) && ($action != "print")) $entry_unconfirmed_row = "bg-danger";
	...
	if (($action != "print") && ($dbTable == "default")) {
		...
		if ($brewer_info[9] == "Y") $entry_paid_display .= "&nbsp;<span class=\"fa fa-star-o\"></span>"; 
	} 
	else { 
		if ($row_log['brewPaid'] == "1") $entry_paid_display .= "<span class=\"fa fa-check text-success\"></span>";
		else $entry_paid_display .= "<span class=\"fa fa-times text-danger\"></span>";
	}
	...
	if (($action != "print") && ($dbTable == "default")) { 
		...
		if ($row_log['brewReceived'] == "1") $entry_received_display .= "<span class=\"fa fa-check text-success\"></span>"; 
	...
	if (($action != "print") && ($dbTable == "default")) {
		...
		$entry_actions .= "<a id=\"modal_window_link\" href=\"".$base_url."output/entry.php?id=".$row_log['id']."&amp;bid=".$row_log['brewBrewerID']."\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Print the Entry Forms for &ldquo;".$row_log['brewName']."&rdquo;\"><span class=\"fa fa-print hidden-xs hidden-sm\"></a> ";
    ...
    <tr class="<?php echo $entry_unconfirmed_row; ?>">
        ...
        <td class="hidden-xs hidden-sm hidden-md"><?php if (!empty($entry_unconfirmed_row)) echo "<a href=\"".$base_url."index.php?section=brew&amp;go=".$go."&amp;filter=".$row_log['brewBrewerID']."&amp;action=edit&amp;id=".$row_log['id']."&amp;view=".$row_log['brewCategory']."-".$row_log['brewSubCategory']."\" data-toggle=\"tooltip\" title=\"Unconfirmed entry\"><span class=\"fa fa-exlamation-circle text-danger\"></span></a> "; echo $row_log['brewName']; ?></td>
